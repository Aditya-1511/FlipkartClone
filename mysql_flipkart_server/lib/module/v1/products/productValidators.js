...
var validateProduct = function (req, res, next) {
  ...
  if (_.isEmpty(price)) {
    errors.push({
      fieldName: "price",
      message: constant.MESSAGES.KEY_CANT_EMPTY.replace("{{key}}", "Price"),
    });
  }
  if (_.isEmpty(color)) {
    errors.push({
      fieldName: "color",
      message: constant.MESSAGES.KEY_CANT_EMPTY.replace("{{key}}", "Color"),
    });
  }
  if (_.isEmpty(size)) {
    errors.push({
      fieldName: "size",
      message: constant.MESSAGES.KEY_CANT_EMPTY.replace("{{key}}", "Size"),
    });
  }
