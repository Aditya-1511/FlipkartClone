...
function addSocketID(socket) {
  ...
  if (userId && socket.payload.isAdmin == 0) {
    userService
      .updateUser({ _id: userId }, { $addToSet: { socketId: socket.id } })
      .bind({})
      .then(function (user) {
        if (user) {
    ...
    adminService
      .update({ _id: userId }, { $addToSet: { socketId: socket.id } })
      .bind({})
      .then(function (user) {
        if (user) {
          console.log("socket Id add");
...
function groupAdd(socket) {
  socket.on("groupAdd", function (matchId) {
    ...
    groupService
      .update(
        { matchId: matchId },
        { $addToSet: { socketId: socket.id, userId: socket.user._id } }
      )
      .bind({})
      .then(function (group) {
...
function socketDisconnections(socket) {
  socket.on("disconnect", function () {
    userService
      .updateUser({ socketId: socket.id }, { $pull: { socketId: socket.id } })
      .bind({})
      .then(function (user) {
...
function socketDisconnectionsGroup(socket) {
  socket.on("disconnect", function () {
    groupService
      .update({ socketId: socket.id }, { $pull: { socketId: socket.id } })
      .bind({})
      .then(function (user) {
...
function groupMessaging(socket) {
  socket.on("groupSend", function (msg, groupId, matchId) {
    groupService
      .getByKey({ _id: groupId, matchId: matchId })
      .bind({})
      .then(function (group) {
        this.group = group;
        ...
        socket.broadcast
          .to(groupId)
          .emit("groupMsg", { msg: msg, group: groupId, user: socket.user });
    ...
    userService
      ...
      .then(function (toUser) {
        ...
        for (var i = 0; i < socketArr.length; i++) {
          socket.broadcast
            .to(socketArr[i])
            .emit("messageRecive", { msg: msg, name: toUser.name });
        }
